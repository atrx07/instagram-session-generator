...
def insta_login_new_session(username, password):
    ...
    headers = {
        "User-Agent": f"Instagram {APP_VERSION} Android ({ANDROID_VERSION}/33; {DPI}dpi; {RESOLUTION}; {MANUFACTURER}; {DEVICE_MODEL}; {DEVICE_MODEL.lower().replace(' ','_')}; {LANGUAGE})",
    ...
    if data.get("authenticated"):
        ...
        console.print(
            f"[cyan]New sessionid (tied to Redmi Note 12 Pro):[/cyan] "
...
# MAIN
def main():
    console.print("[bold magenta]📱 Grab Fresh Redmi Note 12 Pro SessionID[/bold magenta]")
    ...
    if sessionid:
        console.print("[bold green]✅ Save this sessionid for your Redmi device session![/bold green]")
